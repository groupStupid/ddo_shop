							...
							<div>
								...
								<div class="text-center p-4 p-lg-5" style="background: var(--bs-body-bg);border: 1px solid rgb(25,155,221);padding-top: 0px;padding-bottom: 0px;">
									<div style="background: rgb(255,255,255);margin-top: 0px;">
										...
										</div><a id="link" href="#" style="font-size: 12px;">더 보러 가기</a>
									</div>
								</div>
							</div>
							<div>
										...
										<c:if test="${ !empty userGifts }">
											<c:forEach var="val" items="${userGifts}" varStatus="i">
												<div style="background: var(--bs-white);">
													...
													<img src="${val}" id="img${i.index}" style="width: 60%;"><button class="btn btn-primary" id="${i.index}" 
													type="button" onclick="onClickUseGift(this.id)">사용</button></div>
												</div>
											</c:forEach>
											<a id="link-1" href="#" style="font-size: 12px;color: rgb(0,85,255);margin-top: 0px;">더 보러 가기</a>
										</c:if>

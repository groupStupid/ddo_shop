			...
			<div>
				<ul id="boardList" class="list">
					...
					<c:forEach var="boardVO" items="${lists}" varStatus="i">
						...
						<span style="width: 80%;">
							<span class="hit">
								게시글번호 : ${boardVO.eventNum} | 
								<c:if test="${boardVO.eventOngoing.toString() eq '1'}">진행중</c:if>
								<c:if test="${boardVO.eventOngoing.toString() eq '2'}">종료</c:if>
							</span> | 
							<span class="postdate">
								<fmt:formatDate value='${boardVO.eventStart}' pattern="yyyy-MM-dd"/>
								  ~  
								<fmt:formatDate value='${boardVO.eventEnd}' pattern="yyyy-MM-dd"/>
							</span>
						</span>
						</li>
					</c:forEach>
